    ...
    <div class="flex items-center justify-center">
        <div class="overflow-x-auto w-full">
            <div class="w-1/2 mx-auto">
              <h1 class="mb-3 text-2xl">Tabel Kelas</h1>
            <label for="my-modal-3" class="btn my-3">Tambah data</label>
            <form action="{{route('mahasiswa.cari')}}" method="POST" class="my-3">
              @csrf
              <input type="text" name="nama" placeholder="Cari Kelas" class="input input-bordered w-full max-w-xs" required/>
            ...
            <div class="modal">
              <div class="modal-box relative">
                ...
                {{-- Form --}}
                <form action="{{route('kelas.store')}}" method="post" class="my-3">
                  @csrf
                  <div class="form-control w-full max-w-xs">
                    <label class="label">
                      <span class="label-text">Nama Kelas</span>
                    </label>
                    <input type="text" name="kelas" placeholder="Nama Kelas" class="input input-bordered w-full max-w-xs" required/>
                  </div>
                ...
                @foreach ($kelas as $k)
                    <tr>
                        ...
                        <td class="inline-flex">
                            ...
                            <a href="{{route('kelas.delete', ['id' => $k->id])}}">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
